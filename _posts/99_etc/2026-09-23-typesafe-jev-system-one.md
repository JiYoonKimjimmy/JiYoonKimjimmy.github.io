...
> 이 글은 **2026년 9월 기준 `jev-1.13`** 의 공식 문서를 읽고 정리한 것이다. 2026년 9월 15일에 출시된 신생 모델이라 이후 버전에서 달라질 내용이 많다. 성능·비용 수치는 모두 제조사 자체 측정치이며 독립 벤치마크는 아직 없다는 점을 감안하고 읽는 편이 좋다.
...
**둘째, score는 위치지 양이 아니다.** 위 예에서 1.43은 "고객의 43%에게 우회 방법이 없다"는 뜻이 아니다. 단지 레벨 1과 2 사이의 위치일 뿐이다. 문서는 한 걸음 더 나아가, **레벨 사이를 보간해서 실제 수치를 복원하려 하지 말라**고 경고한다. 임계값 통과 여부 판정에는 써도 되지만 수치 자체를 뽑아내는 용도로는 보정이 약하다.
...
다만 자기 일관성(self-consistency)은 보장한다고 한다. 의미적으로 비슷한 입력에는 양적으로 비슷한 출력이 나온다.
...
- **파인튜닝도 LoRA도 없다.** 모든 계정이 같은 가중치를 쓴다. 도메인 적응은 오직 `state`, `instructions`, `criteria`, 그리고 코드에서의 조합으로만 한다. 프롬프트가 곧 설정이다.
...
한국어 고객 문의 분류 같은 데 바로 투입하기에는 검증이 먼저다. 다만 confidence 보정이 실제로 잘 되어 있다면, **낮은 confidence를 사람에게 넘기는 구조**로 감싸서 위험을 통제할 수는 있다. 이건 실제 데이터로 재봐야 아는 영역이다.
