...
- **최상위(top-level)에서만** 선언할 수 있다. 함수 본문 안이나 클래스 내부에는 둘 수 없다.
- 타입 파라미터에 **variance 제어자를 붙일 수 없다.** `typealias Assembler<out P> = ...`는 컴파일 에러다. 이 제약은 6장에서 다시 나온다.
...
가장 실질적인 함정이다. 앞서 본 대로 함수 타입은 반환 공변성을 **공짜로** 갖지만, `fun interface`는 직접 선언해야 한다. `out P`를 안 붙이면 불변(invariant)이 되어 5.2의 스타 프로젝션 대입이 **타입 에러**가 난다. 이 빌더의 핵심 기법이 `out` 한 글자에 의존하게 되는 셈이다.
...
| 오버로드 구분 | 불가 | 가능 |
| 파라미터 이름 | 없음 — `{ m, g -> }` 의 의미를 선언부에서 찾아야 함 | 시그니처에 남고 IDE 힌트 제공 |
| 공변성 | 공짜 | `out` 직접 선언 필요 |
| 호출 | `invoke(m, g)` | `assemble(m, g)` — 의미가 드러남 |
| 기본 구현 추가 | 불가 (붙이면 `Function2` 전체에) | 인터페이스에 default 메서드 추가 가능 |
| 선언 비용 | 1줄 | 3줄 + 클래스 파일 하나 |
| Java에서 보임 | X (`Function2`로 보임) | O |

### 6.3 이 사례에서의 손익

**얻는 것**은 호출부 가독성이다. `assemble(errorMapper, callGuard)`는 두 인자의 역할이 시그니처로 드러난다. `invoke(...)`는 변수명 덕에 읽히는 것이지 타입이 알려주는 게 아니다. 나중에 조립 단계에 공통 동작을 얹을 자리가 생기는 것도 이득이다.
...
1. **공통 의존성 인자가 3~4개로 늘 때** — 이름 없는 위치 인자가 늘면 `{ m, g, t, r -> }`가 해독 불가능해진다.
2. **이 타입이 파일 밖으로 나갈 때** — 외부 모듈이 직접 구현해 넘기는 구조가 되면, 시그니처만 맞으면 통과하는 별칭보다 명목 타입이 안전하다.
3. **같은 시그니처의 다른 개념이 생겨 오버로드가 필요할 때.**
...
| 인자가 많아 위치 인자가 읽히지 않는다 | fun interface |
